      ...
      <div ref={ref} className="relative">
        ...
        {open && (results.influencers.length > 0 || results.tickers.length > 0) && (
          <div className="absolute top-full mt-2 left-0 w-full rounded-xl overflow-hidden z-50"
            style={{backgroundColor:"#ffffff", boxShadow:"0px 8px 24px rgba(23,29,27,0.12)", border:"1px solid #e0ebe6"}}>
            {results.influencers.length > 0 && (
              <div>
                <p style={{fontSize:"10px", fontWeight:600, letterSpacing:"0.1em", color:"#9eb3ae", padding:"8px 16px 4px"}}>INFLUENCERS</p>
                {results.influencers.map((inf: any) => (
                  <div key={inf.id} onClick={() => { router.push(`/influencer/${inf.x_handle.replace(/^@/,"")}`); setOpen(false); setQuery(""); }}
                    ...
                    style={{color:"#171d1b", fontSize:"13px"}}
                    onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.backgroundColor="#f5fbf7"}
                    ...
                    <div>
                      ...
                      <p style={{fontSize:"11px", color:"#9eb3ae"}}>{inf.x_handle}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {results.tickers.length > 0 && (
              <div style={{borderTop: results.influencers.length > 0 ? "1px solid #f0f4f2" : "none"}}>
                <p style={{fontSize:"10px", fontWeight:600, letterSpacing:"0.1em", color:"#9eb3ae", padding:"8px 16px 4px"}}>TICKERS</p>
                {results.tickers.map((t: any) => (
                  <div key={t.ticker} onClick={() => { router.push(`/trending?ticker=${t.ticker}`); setOpen(false); setQuery(""); }}
                    ...
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                      style={{backgroundColor:"#eff5f2", color:"#006859"}}>$</div>
